...
| ID | 需求 | 优先级 | 说明 |
| --- | --- | --- | --- |
| FR-000 | 支持 Beta 快速进入工作区 | P0 | 首次打开应能直接进入可操作工作台，并可访问示例 case 或样例数据。 |
| FR-001 | 支持 case 级工作区 | P0 | 早期 Beta 可以使用默认 case，但要明确范围。 |
| FR-001a | 支持受控内测启动方式 | P0 | 本地演示至少应支持单命令启动；受控环境应有明确访问入口。 |
| FR-002 | 支持 DOCX 上传 | P0 | 原型已支持，首轮保持窄范围。 |
| FR-003 | 展示文件处理状态 | P0 | 包括处理中和失败状态。 |
| FR-003a | 展示系统连接与运行状态 | P0 | 至少包含后端状态、处理状态、模型配置状态、演示 / 真实数据模式。 |
| FR-003b | 展示失败原因与恢复建议 | P0 | 错误提示必须告诉用户发生了什么、影响什么、下一步怎么做。 |
...
| FR-011 | 对选定结果提供 AI 解释 | P1 | 必须引用证据和不确定性；未配置模型时必须明确说明限制，而不是静默失败。 |
...
- 用户首次进入产品后，无需额外解释即可知道先做什么。
- 用户可以查看带来源文本或来源位置的抽取证据。
- 用户可以查看由证据生成的 working cap table。
- 用户可以识别哪些 cap table 行是低置信度或由冲突驱动。
- 用户可以将关键值追溯到来源文档。
- 用户能区分系统不可用、处理中、可继续操作、需要配置模型这几类状态。
- 当后端或模型不可用时，产品仍能提供受限但可理解的体验，或明确阻断并说明原因。
- 系统不得把生成结果呈现为最终法律意见。
- 处理错误要向用户展示，并为调试保留记录。
